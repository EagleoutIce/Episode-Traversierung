...
\tikzset{MTX@BLK/.style={rectangle,minimum size=\mtx@k,inner sep=\z@,outer sep=\z@,fill=#1,draw=gray,line width=\p@}}
...
\endscope
\draw[rounded corners=2\p@,gray,line width=1.9\p@] (.5*\mtx@k, -.5*\mtx@k) rectangle ++(#2*\mtx@k, -#3*\mtx@k);
\endscope}
\makeatother